...
export default function Layout() {
  ...
  return (
    <Wrap>
        ...
        <HeaderInner>
          ...
          <Right>
            ...
            {user ? (
              <>
                <UserBox>
                  <strong style={{ color: "var(--text)", fontSize: 13 }}>{user.name} · {user.rank}</strong>
                  <span>{user.unit} / {user.company} / {user.platoon}</span>
                </UserBox>
      ...
      <Main><Outlet /></Main>
      <Footer>현재 버전은 서버/DB 연결 전 단계의 군 피복·치수 관리 프론트엔드 프로토타입입니다.</Footer>
    </Wrap>
  );
}
