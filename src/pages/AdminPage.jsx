...
function roleLabel(role) { return role === "admin" ? "관리자" : role === "logistics" ? "군수담당" : "병사"; }
...
export default function AdminPage() {
  ...
  async function loadAll() {
    try {
      const [userRows, measurementRows, issueRows] = await Promise.all([getUsers(), getAllMeasurements(), getAllIssues()]);
      setUsers(Array.isArray(userRows) ? userRows : []);
      setRecords(Array.isArray(measurementRows) ? measurementRows : []);
      setIssues(Array.isArray(issueRows) ? issueRows : []);
      setIssueForm((prev) => ({ ...prev, user_id: prev.user_id || String(userRows?.[0]?.id || "") }));
    } catch (err) {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((user) => {
      ...
      const haystack = [user.name, user.username, user.rank, user.unit].join(" ").toLowerCase();
  ...
  if (!canManage) {
    return <Grid><Card><SectionTitle>접근 제한</SectionTitle><SectionDesc>이 페이지는 관리자 또는 군수담당 계정만 접근할 수 있습니다.</SectionDesc></Card></Grid>;
  }
  ...
  return (
    <Grid>
      <Hero>
        <HeroTop>
          <div>
            ...
            <HeroDesc>사용자, 측정 기록, 피복 지급 이력을 실제 서버 DB 기준으로 통합 관리하는 관리자 페이지입니다.</HeroDesc>
          </div>
        </HeroTop>
        <BadgeRow>
      ...
      <CardGrid>
        <StatCard><StatLabel>전체 사용자 수</StatLabel><StatValue>{users.length}</StatValue><StatSub>DB 저장 계정 기준</StatSub></StatCard>
        <StatCard><StatLabel>병사 계정 수</StatLabel><StatValue>{users.filter((u) => u.role === "soldier").length}</StatValue><StatSub>role = soldier</StatSub></StatCard>
        <StatCard><StatLabel>관리 계정 수</StatLabel><StatValue>{users.filter((u) => u.role !== "soldier").length}</StatValue><StatSub>관리자 / 군수담당 포함</StatSub></StatCard>
        <StatCard><StatLabel>측정 / 지급 데이터</StatLabel><StatValue style={{ fontSize: "28px" }}>{records.length} / {issues.length}</StatValue><StatSub>측정 기록 / 지급 이력</StatSub></StatCard>
      </CardGrid>

      <Card>
        <SectionTitle>사용자 검색 및 필터</SectionTitle>
        <SectionDesc>이름, 아이디, 계급, 소속으로 검색하고 역할별로 필터링할 수 있습니다.</SectionDesc>
        <ControlRow>
          <Input placeholder="이름, 아이디, 계급, 소속 검색" value={search} onChange={(e) => setSearch(e.target.value)} />
        ...
        {!filteredUsers.length ? <EmptyState>조건에 맞는 사용자가 없습니다.</EmptyState> : (
          <Table>
            <Row $header $template="1.1fr 0.9fr 0.8fr 1.2fr 1fr 1fr">
              <div>사용자</div><div>역할</div><div>계급</div><div>소속</div><div>최근 키</div><div>측정 건수</div>
            </Row>
            {filteredUsers.map((user) => {
              const latest = latestByUser[user.id];
              const count = records.filter((record) => record.user_id === user.id).length;
              return <Row key={user.id} $template="1.1fr 0.9fr 0.8fr 1.2fr 1fr 1fr"><div><strong>{user.name}</strong><div>{user.username}</div></div><div>{roleLabel(user.role)}</div><div>{user.rank || "-"}</div><div>{user.unit || "-"}</div><div>{latest ? `${mmToCm(latest.height_mm)}cm` : "-"}</div><div>{count}</div></Row>;
            })}
          </Table>
        )}
      </Card>
      ...
      <Card as="form" onSubmit={onCreateIssue}>
        ...
        <SectionDesc>선택한 사용자에게 실제 지급 또는 수동 추천 이력을 저장합니다.</SectionDesc>
        <ControlRow>
          <Select value={issueForm.user_id} onChange={(e) => setIssueForm((prev) => ({ ...prev, user_id: e.target.value }))}>
            {users.map((user) => <option key={user.id} value={user.id}>{user.name} ({user.username})</option>)}
          </Select>
          <Input value={issueForm.item_name} onChange={(e) => setIssueForm((prev) => ({ ...prev, item_name: e.target.value }))} />
          <Input value={issueForm.size} onChange={(e) => setIssueForm((prev) => ({ ...prev, size: e.target.value }))} style={{ minWidth: 120 }} />
          <Input type="number" min="1" value={issueForm.quantity} onChange={(e) => setIssueForm((prev) => ({ ...prev, quantity: e.target.value }))} style={{ minWidth: 90 }} />
          <Select value={issueForm.status} onChange={(e) => setIssueForm((prev) => ({ ...prev, status: e.target.value }))}>
        ...
        <ControlRow>
          <Input placeholder="비고" value={issueForm.note} onChange={(e) => setIssueForm((prev) => ({ ...prev, note: e.target.value }))} style={{ minWidth: 320 }} />
          <Button $primary type="submit">지급 이력 저장</Button>
        </ControlRow>
      ...
      <Card>
        ...
        {!records.length ? <EmptyState>측정 기록이 없습니다.</EmptyState> : (
          <Table>
            <Row $header $template="1fr 1fr 1fr 1fr 1fr"><div>사용자</div><div>측정 시각</div><div>키</div><div>체중</div><div>BMI</div></Row>
            {records.slice(0, 10).map((record) => <Row key={record.id} $template="1fr 1fr 1fr 1fr 1fr"><div>{userMap[record.user_id]?.name || record.user_id}</div><div>{formatDateTime(record.created_at)}</div><div>{mmToCm(record.height_mm)}cm</div><div>{Number(record.weight_kg).toFixed(1)}kg</div><div>{Number(record.bmi).toFixed(2)}</div></Row>)}
          </Table>
        )}
      </Card>
      ...
      <Card>
        ...
        {!issues.length ? <EmptyState>지급 이력이 없습니다.</EmptyState> : (
          <Table>
            <Row $header $template="1fr 1fr 0.8fr 0.8fr 0.8fr 1.2fr"><div>사용자</div><div>품목</div><div>사이즈</div><div>수량</div><div>상태</div><div>지급 시각</div></Row>
            {issues.slice(0, 10).map((issue) => <Row key={issue.id} $template="1fr 1fr 0.8fr 0.8fr 0.8fr 1.2fr"><div>{userMap[issue.user_id]?.name || issue.user_id}</div><div>{issue.item_name}</div><div>{issue.size}</div><div>{issue.quantity}</div><div>{issue.status}</div><div>{formatDateTime(issue.issued_at)}</div></Row>)}
          </Table>
        )}
      </Card>
    </Grid>
